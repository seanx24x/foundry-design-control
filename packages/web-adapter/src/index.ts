...
export function installFoundryInspector(
      ...
      changes.length || rejectedCount
        ? `<div class="review-summary" data-unresolved="${unresolvedCount}" aria-live="polite"><strong>Preparing review…</strong><span>Only included, exactly mapped changes will be sent to your agent.</span></div><div class="review-toolbar"><button data-review-approve-exact>Include exact</button><button data-review-toggle-rejected>${reviewShowRejected ? 'Hide' : 'Show'} removed${rejectedCount ? ` · ${rejectedCount}` : ''}</button></div>${[
            ...
            .map((group) => {
              ...
              return `<section class="review-group" data-review-target="${escapeHtml(target.id)}"><button class="review-group-title" aria-expanded="true"><i data-lucide="chevron-down"></i><strong>${escapeHtml(target.label)}</strong><span class="included-count"></span><span class="group-total">${group.length}</span></button>${group
                .map((change: any) => {
                  const unresolved =
                    change.confidence === 'unresolved' ||
                    (change.mappingCandidates?.length > 1 && !change.selectedMappingId);
  ...
  function renderToolTabs(): void {
    ...
    tabs.innerHTML = categories
      .map(
        (category) =>
          `<button class="tab ${selected && activeCategory === category ? 'active' : ''}" data-category="${category}" data-tooltip="${CATEGORY_LABELS[category]}" title="${CATEGORY_LABELS[category]} controls" aria-label="${CATEGORY_LABELS[category]} controls" ${selected ? '' : 'disabled'}><i data-lucide="${CATEGORY_ICONS[category]}"></i></button>`,
      )
      .join('');
    renderIcons(tabs);
  ...
  function renderContextPanel(category: Category): string {
    ...
    const health =
      category === 'color' && contrast != null
        ? `<div class="design-health ${contrast >= 4.5 ? 'pass' : 'fail'}"><i data-lucide="contrast"></i><span>${contrast}:1 contrast · ${contrast >= 4.5 ? 'AA pass' : 'Needs attention'}</span></div>`
        : '';
    ...
    const categoryTools =
      category === 'layout'
        ? `<div class="context-tools"><button data-layout-action="tidy"><i data-lucide="wand-sparkles"></i>Tidy layout</button><button data-layout-action="up"><i data-lucide="arrow-up"></i>Move up</button><button data-layout-action="down"><i data-lucide="arrow-down"></i>Move down</button><button data-layout-action="lock"><i data-lucide="lock"></i>Lock ratio</button></div>`
        : category === 'typography'
    ...
    const tokenMode =
      (designGraph?.tokens.length ?? 0)
        ? `<div class="context-tools"><button class="${tokenOnly ? 'active' : ''}" data-token-only><i data-lucide="${tokenOnly ? 'lock' : 'unlink-2'}"></i>${tokenOnly ? 'Token-only on' : 'Token-only off'}</button></div>`
        : '';
  ...
  function renderCommands(queryValue = ''): void {
    ...
    commandPalette.querySelector<HTMLElement>('.command-list')!.innerHTML = filtered.length
      ? filtered
          .map(
            (command, index) =>
              `<button class="command-item ${index === 0 ? 'active' : ''}" data-command="${command.id}"><i data-lucide="${command.icon}"></i><span>${command.label}</span><small>${command.shortcut}</small></button>`,
          )
          .join('')
  ...
  function renderHealthPanel(): void {
    ...
    healthList.innerHTML = filtered.length
      ? filtered
          ...
          .join('')
      : `<div class="health-empty"><i data-lucide="${visibleIssues.length ? 'triangle-alert' : 'check'}"></i>${visibleIssues.length ? 'No issues match this filter.' : 'This viewport is looking healthy.'}</div>`;
  ...
  function renderControls(): void {
    ...
    if (activeCategory === 'motion') {
      ...
      controlsRoot.innerHTML = animations.length
        ? `<div class="inspector-heading"><i data-lucide="play"></i><strong>Motion</strong><span class="property-count">${animations.length} active</span></div><div class="motion-list">${animations
            .map((animation, index) => {
    ...
    const remainingSection = remaining.length
      ? `<section class="property-section ${collapsedSections.has(`${activeCategory}:Other`) ? 'collapsed' : ''}" data-section-key="${escapeHtml(`${activeCategory}:Other`)}"><div class="section-head"><button class="section-toggle" aria-expanded="${String(!collapsedSections.has(`${activeCategory}:Other`))}"><i data-lucide="chevron-down"></i><strong>Other</strong></button></div><div class="section-grid">${remaining
