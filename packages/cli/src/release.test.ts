...
test('pins the CLI and MCP bridge to the same release', () => {
  assert.equal(FOUNDRY_VERSION, '0.2.0-beta.19');
  assert.equal(FOUNDRY_PACKAGE_SPEC, 'foundry-design@0.2.0-beta.19');
  assert.equal(FOUNDRY_MCP_PACKAGE_SPEC, 'foundry-design-mcp-server@0.2.0-beta.19');
